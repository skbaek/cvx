import eigenvectors.hermitian_form
...
def has_adjoint (F G : M →ₗ[R] M) (H : hermitian_form R M I) : Prop :=
∀ x y, H (F x) y = H x (G y)

lemma has_adjoint_def {F G : M →ₗ[R] M} (hG : F.has_adjoint G H) : 
  ∀ x y, H (F x) y = H x (G y) := hG

lemma has_adjoint_symm {F G : M →ₗ[R] M} : 
  F.has_adjoint G H ↔ G.has_adjoint F H := 
begin 
  unfold has_adjoint,
  split,
  repeat
  { intros h x y,
    rw [←H.comm y _, ←H.comm _ x, h y x], }
end

lemma has_adjoint_unique_left {F F' G : M →ₗ[R] M}
